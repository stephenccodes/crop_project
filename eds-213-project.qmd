...
What impact does temperature/precip at planting time have on the harvest median date, and total yield for **rice**?
(wheat, barley, millet)
...
fao_clean <- fao %>% 
  ...
  filter((unit %in% "kg/ha"),(crop %in% c("Rice", "Millet", "Barley", "Wheat", "Rye") ) )
...
# ```{r}
# df1 <- data.frame(colnames(fao_clean))
# df2 <- data.frame(colnames(crop_calendar_clean))
# ```
# 
# ```{r}
# common_values <- intersect(df1$colnames.fao_clean., df2$colnames.crop_calendar_clean.)
# ```
